...
class visualizer():
    ...
    def show_pulse(self, n_records=0, save=False, filename='pulse.png'):

        for i in range(len(self.event_records[n_records])):
            record_length = self.event_records[n_records][i]['length']
            plt.plot(np.arange(i * record_length, (i + 1) * record_length), self.event_records[n_records][i]['data'],
                     label='record ' + str(i))

        print('ch.', self.event_records[n_records][0]['channel'])
        print(self.event_records[n_records][0]['time'], 'ns')

        plt.xlabel('Time (2ns bins)')
        plt.ylabel('ADC in nT DAQ')
        plt.legend()
